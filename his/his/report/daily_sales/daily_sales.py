...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		if posting_date not in data:
			# Initialize a new row for the posting_date if not already present
			data[posting_date] = {
		...
		# Aggregate details for the posting_date
		
		data[posting_date]["sales_order"].update(invoice_so_dn_map.get(inv.name, {}).get("sales_order", []))
		data[posting_date]["delivery_note"].update(invoice_so_dn_map.get(inv.name, {}).get("delivery_note", []))
		data[posting_date]["cost_center"].update(invoice_cc_wh_map.get(inv.name, {}).get("cost_center", []))
		data[posting_date]["warehouse"].update(invoice_cc_wh_map.get(inv.name, {}).get("warehouse", []))
		data[posting_date]["mode_of_payment"].update(mode_of_payments.get(inv.name, []))
		...
		for income_acc in income_accounts:
			...
			data[posting_date][frappe.scrub(income_acc)] = data[posting_date].get(frappe.scrub(income_acc), 0) + income_amount

		# Aggregate unrealized account amounts
		for account in unrealized_profit_loss_accounts:
			unrealized_amount = flt(internal_invoice_map.get((inv.name, account)))
			data[posting_date][frappe.scrub(account + "_unrealized")] = data[posting_date].get(frappe.scrub(account + "_unrealized"), 0) + unrealized_amount
		...
		for tax_acc in tax_accounts:
			if tax_acc not in income_accounts:
				...
				data[posting_date][frappe.scrub(tax_acc)] = data[posting_date].get(frappe.scrub(tax_acc), 0) + tax_amount

		# Update aggregated fields
		data[posting_date]["net_total"] += base_net_total or inv.base_net_total
		data[posting_date]["tax_total"] += total_tax
	...
	final_data = []
	for posting_date, row in data.items():
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns += [
		...
		{"label": _("Date"), "fieldname": "posting_date", "fieldtype": "Data", "width": 150},
